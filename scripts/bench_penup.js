...
for (line = 0; line < LINES; line++) {
  for (k = 0; k < PER_LINE; k++) {
    ...
    for (st = 1; st <= 10; st++) {
      app.tick(16);
      app.moveTo(id, x + st * 4, y - 24 + TOP + st * 2.4);
    }
    count.seg = 0; count.canvas = 0;
    app.up(id);
    app.tick(40);
    app.flushFrames();
    if (k === PER_LINE - 1) {
      rows.push({ line: line + 1, on: app.strokes().length,
                  seg: count.seg, canvas: count.canvas });
    }
    id++;
  }
}
...
console.log('\n  the cost of ONE pen-up, as the page fills up\n');
console.log('  line   strokes on page   curves rasterised   new canvases    ');
var i;
